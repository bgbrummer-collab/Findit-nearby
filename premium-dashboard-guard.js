...
document.addEventListener('click',capturePlanClick,true);document.addEventListener('change',captureRadiusChange,true);window.addEventListener('storage',qualitySync);document.addEventListener('findit:results-rendered',qualitySync);document.addEventListener('findit:dashboard-sync',qualitySync);new MutationObserver(qualitySync).observe(document.documentElement,{childList:true,subtree:true});setTimeout(qualitySync,0);setTimeout(qualitySync,500);setTimeout(qualitySync,1500);
})();
