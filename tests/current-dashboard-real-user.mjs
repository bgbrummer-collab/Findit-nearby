...
async function run(viewport,label){
 ...
 await check(`${label} Ask FindIt opens`,async()=>{await closeFx(page);const b=page.locator('#finditExactShell [data-fx="assistant"]:visible').last();if(!await b.count())return;await b.click({timeout:5000});await page.waitForTimeout(300);const visible=await page.locator('#assistantPanel,#assistantModal,.assistant-panel,.assistant-modal').filter({visible:true}).count().catch(()=>0);const stable=await page.locator('#fxStableModal').isVisible().catch(()=>false);if(!visible&&!stable)throw Error('assistant did not open');await closeFx(page)});
