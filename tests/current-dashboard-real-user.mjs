...
async function run(viewport,label){
 ...
 await check(`${label} Ask FindIt entry point responds`,async()=>{await closeFx(page);const b=page.locator('#finditExactShell [data-fx="assistant"]:visible').last();if(!await b.count())throw Error('assistant action missing');let dialogSeen=false;page.once('dialog',async d=>{dialogSeen=true;await d.dismiss()});await b.click({timeout:5000});await page.waitForTimeout(300);const visible=await page.locator('#assistantPanel:not(.hidden),#fxStableModal:not(.hidden),#fxCompleteModal:not(.hidden),#v10UniversalModal:not(.hidden)').count();if(!dialogSeen&&!visible)throw Error('assistant/search entry point produced no usable UI');await closeFx(page)});
