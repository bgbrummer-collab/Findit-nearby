...
async function loadProductIntelligence(i){
  ...
  try{
    ...
    el.innerHTML=d.offers.map(o=>{const retailer=o.retailer?.name||"Retailer";const price=o.price==null?"Price unavailable":new Intl.NumberFormat("en-ZA",{style:"currency",currency:o.currency||"ZAR"}).format(Number(o.price));const stock=o.availability||"Availability not supplied";return `<article class="pi-offer ${o.verified?"verified":""}"><div><h4>${esc(o.product_name||d.bestProduct?.name||"Product")}</h4><p>${esc(retailer)}</p><div class="pi-meta"><span>${o.verified?"✓ Verified listing":"Catalog listing"}</span><span>${esc(stock)}</span>${o.source?`<span>${esc(o.source)}</span>`:""}</div><div class="pi-actions">${validUrl(o.product_url)?`<a href="${esc(o.product_url)}" target="_blank" rel="noopener noreferrer">View product</a>`:""}</div></div><div class="pi-price">${esc(price)}</div></article>`}).join("");
