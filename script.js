...
function syncExactDashboardResult(data){
...
async function finditSearchRequest(fd){const c=new AbortController(),t=setTimeout(()=>c.abort(),36000);try{return await fetch('/api/search',{method:'POST',body:fd,signal:c.signal})}catch(e){if(e?.name==='AbortError')throw Error('Image identification took too long. Please try again in a moment.');throw e}finally{clearTimeout(t)}}

searchBtn.onclick=async()=>{if(!state.file)return;resetResults();searchBtn.disabled=true;showSearchOverlay();try{if(!state.coords){try{state.coords=await getLocation();locationBtn.textContent='✓ Location ready'}catch{}}const fd=new FormData();fd.append('image',state.file);if(state.coords){fd.append('lat',state.coords.lat);fd.append('lon',state.coords.lon)}let r=await finditSearchRequest(fd);let data=await r.json().catch(()=>({}));if(!r.ok)throw Error(data.message||data.error||`Search failed (${r.status})`);if(!data.identification&&data.requiresUserInput){hideSearchOverlay();const typed=window.prompt('FindIt received the photo but cannot safely identify it automatically yet. Enter the product name or barcode shown on the product:','');if(typed&&typed.trim()){const tf=new FormData();tf.append('query',typed.trim());r=await finditSearchRequest(tf);data=await r.json().catch(()=>({}));if(!r.ok)throw Error(data.message||data.error||`Search failed (${r.status})`)}else{throw Error('Product name or barcode is needed to continue without guessing.')}}state.result=data;renderIdentification(data.identification||{});syncExactDashboardResult(data);try{document.dispatchEvent(new CustomEvent('findit:results-rendered',{detail:{result:data}}))}catch{}loadProductIntelligence(data.identification||{});state.offers=Array.isArray(data.offers)?data.offers:[];state.diagnostics.exactOfferCount=state.offers.length;state.diagnostics.exactProductMatch=state.offers.length>0;renderOffers();renderFreeActions(data.identification||{});const conf=Number(data.identification?.confidence||0);if(data.blocked)showWarning(data.message||'This item cannot be searched.',true);else if(conf<.55)showWarning('FindIt is not confident enough to guess. Try a clearer photo.');if(state.coords&&data.identification&&conf>=.55&&!data.blocked){await loadNearby(data.identification,state.radius);try{document.dispatchEvent(new CustomEvent('findit:nearby-updated'))}catch{}}else{state.stores=[];renderStores();updateMap();try{document.dispatchEvent(new CustomEvent('findit:nearby-updated',{detail:{stores:[]}}))}catch{}showNothing(data.identification?'Allow location to see nearby retailers.':'FindIt needs a product identity before it can show relevant nearby retailers.');}saveRecent(data.identification||{});trackFindIt('search_complete',{success:true});results.classList.remove('hidden');results.scrollIntoView({behavior:'smooth'});setStatus('Search complete.')}catch(e){state.diagnostics.lastError=String(e.message||e).slice(0,240);results.classList.remove('hidden');showWarning(`Search error: ${e.message}`,true);trackFindIt('search_failed',{success:false});setStatus('Search failed. Check the message below.',true)}finally{hideSearchOverlay();searchBtn.disabled=!state.file}};
