...
function openFeature(){const m=modal();m.classList.remove('hidden');m.setAttribute('aria-hidden','false');setTimeout(()=>$('#fxFeatureTitle')?.focus(),50)}
function injectDashboard(){
 const shell=$('#finditExactShell');if(!shell)return false;
 const nav=shell.querySelector('.fx-nav');
 if(nav&&!nav.querySelector('[data-fx="suggest-feature"]')){const b=document.createElement('button');b.type='button';b.dataset.fx='suggest-feature';b.innerHTML='✦ <span>Suggest a Feature</span>';const feedback=nav.querySelector('[data-fxnav="feedback"]');feedback?.after(b)||nav.appendChild(b)}
...
function sync(){wireFeedback();injectDashboard();injectFeedbackCta()}
