...
const RETAILERS=[
  ...
  {name:"Sportsmans Warehouse",domain:"sportsmanswarehouse.co.za",cats:["sports","footwear","shoes","fitness","outdoor","cycling"]},
...
function queryFrom(b){return [b.brand,b.model,b.name,b.object,b.query].map(x=>String(x||"").trim()).filter(Boolean).filter((x,i,a)=>a.findIndex(y=>y.toLowerCase()===x.toLowerCase())===i).join(" ").replace(/\s+/g," ").slice(0,180)}
...
export default function handler(req,res){
  ...
  return res.json({ok:true,query:q,links,message:"These are retailer search links, not verified exact-product or branch-stock claims."});
}
