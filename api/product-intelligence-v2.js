...
const TYPE_GROUPS=[
  ...
  ['hammer',['hammer']],
  ['jacket',['jacket','coat']],
  ['shirt',['shirt','t shirt','tshirt','top']],
  ['jeans',['jeans','denim']],
...
function typeAliases(body){
  const source=norm([body.object,body.category,body.retailCategory,body.name].filter(Boolean).join(' '));
  for(const [,aliases] of TYPE_GROUPS)if(aliases.some(a=>source.includes(norm(a))))return aliases.map(norm);
  return [];
}
...
function exactEnough(title,url,body){
  const hay=norm(`${title||''} ${url||''}`),brand=norm(body.brand);
  ...
  return hits>=Math.min(5,Math.max(2,Math.ceil(wanted.length*.4)));
}
function urlLooksExact(o,body){
  ...
  try{
    ...
    if(!typeCompatible(path,body))return false;
    ...
    return hits>=Math.min(4,Math.max(2,Math.ceil(q.length*.4)));
  }catch{return false}
}
function sanitizeExact(result,body){
  ...
  if(body.brand){const brand=norm(body.brand);offers=offers.filter(o=>norm(`${o.product_name||''} ${o.product_url||''}`).includes(brand));}
