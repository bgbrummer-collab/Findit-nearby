...
function awinClean(v,n=2000){return v==null?null:String(v).trim().slice(0,n)}
...
async function awinImportHandler(req,res){
  ...
  try{
    ...
    const mapped=rows.map(r=>({
      sku:awinClean(r.merchant_product_id||r.aw_product_id,120),
      name:awinClean(r.product_name,300),
      description:awinClean(r.description,2000),
      category:awinClean(r.merchant_category||r.category_name,250),
      image_url:awinClean(r.merchant_image_url||r.aw_image_url,2500),
      price:num(r.search_price||r.store_price),
      store_price:num(r.store_price),
      currency:awinClean(r.currency||"ZAR",12)||"ZAR",
      external_product_id:awinClean(r.aw_product_id||r.merchant_product_id,120),
      product_url:awinClean(r.aw_deep_link||r.merchant_deep_link,3000),
      merchant_url:awinClean(r.merchant_deep_link,3000),
      source_updated_at:iso(r.last_updated),
      brand:awinClean((r.product_name||"").split(/\s+/).slice(0,2).join(" "),160)
