(()=>{
  ...
  function sync(){
    ...
    if(b){b.textContent=isPremium()?'Return to Free for testing':'Test Premium checkout — R99/month';}
  ...
  function returnFree(){
    ...
    $('#premiumHome')?.classList.add('hidden');
    $('#v10CommandCentre')?.classList.add('hidden');
    $('#premiumStatusBadge')?.classList.add('hidden');
    $('#premiumWorkspaceButton')?.classList.add('hidden');
    const nav=$('#premiumDrawerNav');if(nav)nav.style.display='none';
    sync();
    $('#premiumModal')?.classList.add('hidden');
    window.scrollTo({top:0,behavior:'smooth'});
  }
  async function checkout(){
    const email=window.prompt('Enter the email to use for the FindIt Premium test subscription:');
    if(!email)return;
    const r=await fetch('/api/paystack-init',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email})});
    const d=await r.json().catch(()=>({}));
    if(!r.ok||!d.authorization_url){alert(d.error||'Could not start Premium test checkout.');return}
    if(d.reference)sessionStorage.setItem('findit_pending_paystack_reference',d.reference);
    location.assign(d.authorization_url);
  }
  document.addEventListener('click',e=>{
    const free=e.target.closest?.('#finditTestFreeBtn');
    if(free){e.preventDefault();e.stopImmediatePropagation();returnFree();return}
    const b=e.target.closest?.('#activatePremiumTester');
    if(!b)return;
    e.preventDefault();e.stopImmediatePropagation();
    if(isPremium())returnFree();else checkout();
  },true);
