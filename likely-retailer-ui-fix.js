(()=>{
 ...
 function paint(){
  ...
  if(!Array.isArray(state.stores)||!state.stores.length){nearbyStores.innerHTML='';return}
  const verified=state.stores.filter(s=>s.exactProductMatch&&s.stockVerified);
  state.stores=verified;
  nearbyStores.innerHTML=verified.map((s,i)=>{
   const directions=`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(`${s.lat},${s.lon}`)}`;
   return `<article class="store-card verified-store" data-store="${i}"><span class="store-rank">${i+1}</span><div class="store-main"><strong>${esc2(s.name)}</strong><small>${esc2(s.address||s.type||'Retailer')}</small><div class="store-tags"><span>✓ Exact item verified</span><span>${esc2(s.stockStatus||'Stock verified')}</span><span>${esc2(s.source||'Connected retailer data')}</span></div>${typeof premiumState!=='undefined'&&premiumState.active?`<label class="premium-compare-check"><input type="checkbox" data-compare-store="${i}" ${typeof premiumCompareSelection!=='undefined'&&premiumCompareSelection.has(i)?'checked':''}> Compare</label>`:''}</div><div class="store-side"><div class="store-distance">${Number(s.distanceKm).toFixed(1)} km</div><div class="store-actions">${s.phone?`<a href="tel:${esc2(s.phone)}">Call</a>`:''}${valid2(s.website)?`<a href="${esc2(s.website)}" target="_blank" rel="noopener noreferrer">Website</a>`:''}<a href="${directions}" target="_blank" rel="noopener noreferrer">Directions</a></div></div></article>`
  }).join('');
 ...
 function install(){try{const original=window.renderStores;window.renderStores=function(){try{if(typeof original==='function')original()}catch{};try{paint()}catch(e){console.warn('FindIt exact-retailer paint failed',e)}};if(typeof renderStores!=='undefined')renderStores=window.renderStores}catch{}}
 install();
})();