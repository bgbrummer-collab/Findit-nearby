...
(()=>{
  ...
  const loadFindActionsHours=()=>loadScript('find-actions-hours','find-actions-hours.js?v=20260914-actions1',()=>window.__finditFindActionsHours);
  ...
  async function loadGuards(){await loadDashboardAudit();await loadFeedbackUi();await loadSmartChoice();await loadShoppingAssistant();await loadShoppingOfferGuard();await loadShoppingStoreAccess();await loadFindActionsHours();await loadActionOwner();await Promise.all([loadPolish(),loadPriceSweep(),loadRelevance(),loadCommerce(),loadStructure(),loadProductGuard(),loadBuyingContext(),loadLocalMarket(),loadExactnessGuard()]);await loadCommerceUiV4();await loadCompare()}
