...
(()=>{
  ...
  function loadFinalProductFix(){
    if(window.__finditFinalProductExperienceFix||document.querySelector('script[data-findit-final-product-fix]'))return;
    const f=document.createElement('script');f.src='final-product-experience-fix.js?v=20260909-final1';f.async=false;f.dataset.finditFinalProductFix='1';document.head.appendChild(f);
  }
  ...
  async function loadDashboardRuntime(){
    ...
    s.onload=()=>{dashboardLoading=false;loadFinalProductFix();try{document.dispatchEvent(new CustomEvent('findit:dashboard-sync'))}catch{}};
    s.onerror=()=>{dashboardLoading=false};document.head.appendChild(s);
  }
  loadModalPolish();
  loadClickGuard();
  const observer=new MutationObserver(()=>{if(document.querySelector('#finditExactShell')){loadDashboardRuntime();loadFinalProductFix();observer.disconnect()}});observer.observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(loadDashboardRuntime,0);setTimeout(loadDashboardRuntime,800);setTimeout(loadFinalProductFix,1200);
  ...
  document.addEventListener('findit:results-rendered',()=>{setTimeout(loadResearchRuntime,0);setTimeout(loadFinalProductFix,0)});
})();
