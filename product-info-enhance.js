...
(()=>{
  ...
  function loadRetailerRelevance(){
    ...
    const f=document.createElement('script');f.src='dashboard-retailer-relevance-fix.js?v=20260909-relevance1';f.async=false;f.dataset.finditRetailerRelevance='1';document.head.appendChild(f);
  }
  function loadModalPolish(){
    ...
    if(!window.__finditCompareStockReliability&&!document.querySelector('script[data-findit-compare-stock-reliability]')){const c=document.createElement('script');c.src='compare-stock-reliability-fix.js?v=20260909-pricestock2';c.async=false;c.dataset.finditCompareStockReliability='1';document.head.appendChild(c)}
    loadRetailerRelevance();
  }
  function loadClickGuard(){
    ...
    guardPromise=new Promise(resolve=>{
      ...
      const g=document.createElement('script');g.src='product-info-click-fix.js?v=20260909-instant1';g.async=false;g.dataset.finditProductClickFix='1';g.onload=resolve;g.onerror=resolve;document.head.appendChild(g);
    });
    return guardPromise;
  }
  async function loadDashboardRuntime(){
    ...
    s.onload=()=>{dashboardLoading=false;loadRetailerRelevance();try{document.dispatchEvent(new CustomEvent('findit:dashboard-sync'))}catch{}};
    s.onerror=()=>{dashboardLoading=false};document.head.appendChild(s);
  }
  loadModalPolish();
  loadClickGuard();
  const observer=new MutationObserver(()=>{if(document.querySelector('#finditExactShell')){loadDashboardRuntime();loadRetailerRelevance();observer.disconnect()}});observer.observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(loadDashboardRuntime,0);setTimeout(loadDashboardRuntime,800);setTimeout(loadRetailerRelevance,1200);
  ...
  document.addEventListener('findit:results-rendered',()=>{setTimeout(loadResearchRuntime,0);setTimeout(loadRetailerRelevance,0)});
})();
