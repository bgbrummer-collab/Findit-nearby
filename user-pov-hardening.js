...
function repairOffer(o){
 if(!o||typeof o!=='object')return false;let changed=false;
 for(const k of ['product_name','name','title','description','source'])if(typeof o[k]==='string'){const d=decode(o[k]);if(d!==o[k]){o[k]=d;changed=true}}
 const before=clean(o?.retailer?.name||o?.retailer||o?.store||o?.seller||''),r=retailer(o);
 if(before!==r){if(o.retailer&&typeof o.retailer==='object')o.retailer={...o.retailer,name:r};else o.retailer=r;changed=true}
 o._finditRetailerLabel=r;return changed;
}
function repairOffers(){let changed=false;const s=state();if(Array.isArray(s.offers))for(const o of s.offers)changed=repairOffer(o)||changed;if(Array.isArray(window.productIntelligence?.offers))for(const o of window.productIntelligence.offers)changed=repairOffer(o)||changed;return changed}
...
function renderTopStores(){
 ...
 if(!rows.length){host.dataset.userStoreSig='';const i=state()?.result?.identification,c=state()?.coords;if(i&&(c?.lat!=null||c?.latitude!=null)&&!/Finding relevant nearby retailers/i.test(host.textContent||''))host.innerHTML='<div class="fx-user-top-empty">Finding relevant nearby retailers…</div>';return}
 const sig=rows.slice(0,5).map(s=>norm(`${s.name}|${s.address}|${s.distanceKm}|${storeStatus(s)}`)).join('||');if(host.dataset.userStoreSig===sig)return;host.dataset.userStoreSig=sig;
...
const benefit=/\b(offers?|provides?|features?|uses?|has|includes?|helps?|supports?|improves?|gives?|designed|made|cushion|comfort|durab\w*|traction|support|reduces?|protects?|connects?|records?|adjustable|portable|reliable|lightweight|breathable|grip|moistur\w*|detangl\w*|noise|battery|performance)\b/i;
...
function run(){queued=false;styles();const offersChanged=repairOffers(),shoppingChanged=sanitizeStoredShopping();renderTopStores();syncSmartChoice();cleanProductInfo();cleanCommerce();if(shoppingChanged){window.finditShoppingAssistantRefresh?.();setTimeout(()=>window.finditUserFirstPolish?.(),30)}if(offersChanged)setTimeout(()=>{window.finditSmartChoiceRefresh?.();setTimeout(()=>window.finditRefreshFindActions?.(),30)},0)}
