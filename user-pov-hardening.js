...
function run(){queued=false;styles();repairOffers();const changed=sanitizeStoredShopping();renderTopStores();syncSmartChoice();cleanProductInfo();cleanCommerce();if(changed){window.finditShoppingAssistantRefresh?.();setTimeout(()=>window.finditUserFirstPolish?.(),30)}setTimeout(()=>window.finditSmartChoiceRefresh?.(),0)}
