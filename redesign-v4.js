...
function init(){document.body.classList.add('findit-exact-dashboard');createShell();setTimeout(()=>{wireUploads();syncAll()},500);document.addEventListener('findit:results-rendered',syncAll);document.addEventListener('findit:nearby-updated',syncAll);window.addEventListener('storage',syncPremium)}
