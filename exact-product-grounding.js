(()=>{
 ...
 window.fetch=async function(input,init){
  ...
  try{
   ...
   id.exactWebChecked=true;id.exactWebConfidence=Number(g.confidence||0);id.exactWebSources=Array.isArray(g.sources)?g.sources:[];id.exactWebEvidence=Array.isArray(g.evidence)?g.evidence:[];
   if(g.exactFound){
    ...
    const detailed=clean(g.searchQuery);if(detailed&&detailed.length>clean(id.searchQuery).length)id.searchQuery=detailed;id.matchLevel=id.modelEvidence?'model-unverified':id.brandEvidence?'brand-level':'descriptive-unverified';id.exactIdentityVerified=false;base.message='FindIt identified the physical item, but could not verify an exact product identity from live web evidence. It will not pretend a generic category match is the exact item.';
   }
