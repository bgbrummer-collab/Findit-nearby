(()=>{
 ...
 function bindControls(){
  const free=$('#finditSwitchFree');if(free&&!free.dataset.bound){free.dataset.bound='1';free.addEventListener('click',e=>{e.preventDefault();localStorage.removeItem(KEY);localStorage.removeItem('findit_premium_payment_reference');location.reload()})}
 ...
 function sync(){
  ...
  const wanted=active()?'<button id="finditSwitchFree">Switch to Free (testing)</button><button id="finditManagePremium">Manage Premium</button>':'<button id="finditStartCheckout" class="primary">Test R99/month checkout</button>';
